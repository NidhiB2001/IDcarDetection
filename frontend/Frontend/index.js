...
function enableCam(event) {
  ...
  const constraints = {
    // for rear camera in mobile...
    video: {width:screen.width, height:screen.height, frameRate: {ideal: 1}, facingMode: 'environment' }           
  };
  ...
  navigator.mediaDevices.getUserMedia(constraints).then(function (stream) {
    ...
    video.addEventListener("loadeddata", qrpredictWebcam);
  });
}
...
function qrpredictWebcam() {
  ...
  window.requestAnimationFrame(qrpredictWebcam);
}
...
const file = new File([blob], `crop_${new Date().getTime()}.jpg`);
  let formData = new FormData()
  formData.append('file', file)

  fetch("http://192.168.0.239:8011/input",{
      body: formData,
      method: "post"
    });
}